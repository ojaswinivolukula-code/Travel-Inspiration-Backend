...
export const getPlaceById = async (id) => {
  return await supabase.from("places").select("*").eq("id", id).single();
};

// Create place (Admin)
export const createPlace = async (placeData) => {
  return await supabase.from("places").insert([placeData]).select();
};

// Update place (Admin)
export const updatePlace = async (id, placeData) => {
  return await supabase.from("places").update(placeData).eq("id", id).select();
};
